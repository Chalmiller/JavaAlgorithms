...
public class AList<T> implements ListInterface<T> 
{
   ...
   /** public int getLastIndex(T item) 
     * returns the index of the last entry which is equal to item. 
   ...
   /** public boolean equals(Object other) 
     *  that returns true when the contents of 2 AList objects are the same. 
     *  Note that 2 AList objects are the same if they have the same number 
     *  of items and each item in one object is equal to the item in its 
     *  corresponding location in the other object.
     */
   public boolean equals(Object other) {
       
        assert (other instanceof AList);
        
        AList list2 = (AList) other;

        if(this.getLength() == list2.getLength()) {
            for (int index = 0; index < numberOfEntries; index ++) {
                if (this.list[index] != list2.list[index]) {
                    return false;
                }
            }
        }
   ...
   /**  public int removeEvery(T item) 
     *  removes all occurrences of item and returns the number of removed 
     *  items. 
     */
   public int removeEvery(T item) {
